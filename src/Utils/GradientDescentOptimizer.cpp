#include <Utils/GradientDescentOptimizer.h>
#include <time.h>
#include <stdio.h>
...
#endif
		(*X)[i] = initialValue;
//		printf("gradient[%d] = %lf (ep = %lf, eN = %lf)\n", i, (*gradient)[i], eP, eN);
	}
}
...
//		printf("trying alpha=%lf --> objFunc = %lf\n", alpha, newObjValue);
...
int GradientDescentOptimizer::optimize(DynamicArray<double>* X){
	...
	printf("Initial objective value: %lf\n", curObjValue);
	...
	while (!done){
		...
		if (alpha>0){
			printf("Better solution found at alpha=%lf! New obj value: %lf\n", alpha, curObjValue);
	...
	printf("Done! Optimization took %ds\n", timeEllapsed);
