...
RigidBody::~RigidBody(void){
	...
	for (unsigned int i=0;i<cdps.size();i++)
		delete cdps[i];
...
*/
unsigned int RigidBody::renderToObjFile(FILE* /* fp */, unsigned int /* vertexIdxOffset */) {
	int retVal = 0;
//	for (unsigned int i=0;i<meshes.size();i++)
...
*/
void RigidBody::loadFromFile(FILE* f, const char* sPath){
	if (f == NULL)
...
*/
void RigidBody::addMeshObj(const char* objFilename, const Vector3d& offset, const Vector3d& scale ) {
	GLMesh* tmpMesh;
