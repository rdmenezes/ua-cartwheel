...
void SimulationInterface::init_simulation(vector<StartStatePtr> const& start_state, vector<BoxStartStatePtr> const& boxes)
{
  fullClear();

  simulator_->addObject("ground", "data/objects/flatGround.rbs", -1);

  for (vector<BoxStartStatePtr>::const_iterator bx = boxes.begin(); bx != boxes.end(); ++bx)
  {
...
void SimulationInterface::simulate(vector<StartStatePtr> const &start_state,
                                   vector<BoxStartStatePtr> const& boxes,
...
void SimulationInterface::simulate(vector<StartStatePtr> const &start_state,
                                   vector<vector<ExtendedActionPtr> > const &actions)
{
  vector<BoxStartStatePtr> no_boxes;
