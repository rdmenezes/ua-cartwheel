#include <Core/BipV3BalanceController.h>
...
bool BipV3BalanceController::runASimulationStep(){
	...
	Vector3d leftFootPos = con->characterFrame.inverseRotate(Vector3d(con->root->getCMPosition(),con->lFoot->getCMPosition()));
	Vector3d rightFootPos = con->characterFrame.inverseRotate(Vector3d(con->root->getCMPosition(), con->rFoot->getCMPosition()));
