#include <Core/ControllerPerturbator.h>
...
//constructor...
Perturbation::Perturbation(double* var, const char *PName, double w){
	if (var == NULL)
...
const char* Perturbation::getName(){
	return name;
}
...
void ControllerPerturbator::perturb(int which, double value){
	if (which>=0 && (unsigned int)which<perturbations.size())
		perturbations[which]->setValue(value);
}

//adds a new perturbation to the collection...
void ControllerPerturbator::addPerturbation(DynamicArray<double*> *vars, const char* PName, double w){
	if (vars->size()<=0)
		return;
...
//adds a new perturbation to the collection...
void ControllerPerturbator::addPerturbation(double *var, const char* PName, double w){
...
void ControllerPerturbator::setDeltaP(DynamicArray<double> *deltaP){
	for (unsigned int i=0;i<deltaP->size();i++)
