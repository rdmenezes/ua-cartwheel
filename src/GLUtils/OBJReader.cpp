...
*/
Point3d readCoordinates(const char* line){
	Point3d p;
	int n = sscanf(line, "%lf %lf %lf", &p.x, &p.y, &p.z);
    n += 1; // kills compiler warning; will be optimized away
	return p;
}
...
//	Logger::out()<< "Loading mesh: " << fileName <<std::endl;
	...
	//printf("Found file %s and beginning to load\n", fileName);
	...
	while (!feof(f)){
		...
		if (lineType == FACE_INFO){
			...
			if (temporaryPolygon.indexes.size() == 0)
				printf("Found a polygon with zero vertices.\n");
			else
