...
void Joint::fixJointConstraints(bool fixOrientations, bool fixVelocities, bool recursive) {
    ...
    std::string sThisName = name;
    if (recursive)
        for (unsigned int i = 0; i < child->cJoints.size(); i++) {
            if(sThisName.compare(0, 4, "ext_")) {
//                printf("This is joint [%s], now fixing [%s].\n", name, child->cJoints[i]->getName());
                child->cJoints[i]->fixJointConstraints(fixOrientations, fixVelocities, recursive);
            } else {
//                printf("*** Last joint found!!! (due to: This is joint [%s], now fixing [%s])\n", name, child->cJoints[i]->getName());
                return;
            }
        }
}
 ...
 */
void Joint::readAxes(const char* /* axes */) {
}
 ...
 */
void Joint::readJointLimits(const char* /* limits */) {
}
...
/**
        set the chil
 */
void Joint::setChild2(ArticulatedRigidBody* child) {
    if (this->child != NULL)
        throwError("This joint already has a child");
    this->child = child;
    child->cJoints.push_back(this);
}
