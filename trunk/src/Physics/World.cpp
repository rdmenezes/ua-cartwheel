...
void World::destroyWorld() {
    ...
    for (unsigned int i = 0; i < AFs.size(); i++)
        delete AFs[i];
    AFs.clear();
 ...
 */
unsigned int World::renderRBsToObjFile(FILE* fp, unsigned int vertexIdxOffset) {
 ...
 */
ArticulatedRigidBody* World::getARBByName(const char* name, const char* articulatedFigureName) {
    if (name == NULL)
        return NULL;
    for (unsigned int i = 0; i < ABs.size(); i++)
...
/**
        ...
        its name and its articulared figure name, or NULL if it is not found
 */
ArticulatedRigidBody* World::getARBByName(const char* name, ArticulatedFigure* articulatedFigure) {
    if (name == NULL)
        return NULL;
    for (unsigned int i = 0; i < ABs.size(); i++)
 ...
 */
RigidBody* World::getRBByName(const char* name) {
    if (name == NULL)
        return NULL;
...
RigidBody* World::getRB(int i) {
    return objects[i];
}

int World::getRBCount() {
