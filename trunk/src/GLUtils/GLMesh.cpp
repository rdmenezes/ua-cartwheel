...
/*	Replace this... store the indexes of the two neighbours, instead of storing the way to retrieve them... 
	...
	for (int i=0;i<vertexCount;i++){
		...
		if (sharedVertices[i] == NULL)
			continue;
		for (unsigned int j=0;j<sharedVertices[i]->vertexInstances.size();j++){
...
void GLMesh::drawMesh(bool useColours){
	...
	for (unsigned int i = 0;i<polygons->categories.size();i++){
...
*/	
unsigned int GLMesh::renderToObjFile( FILE* fp, unsigned int vertexIdxOffset, TransformationMatrix toWorld ) {

	Point3d vertex;
	unsigned int nbVerts = vertexList.size()/3;
	...
	for ( unsigned int i = 0; i<polygons->categories.size(); i++ ){
		GLPolyIndexList* tempIndexList = polygons->categories[i];
		unsigned int vertsPerFace = tempIndexList->polyVertexCount;
		unsigned int nbFaces = tempIndexList->indexList.size() / vertsPerFace;
		unsigned int idx = 0;
		for ( unsigned int j = 0; j<nbFaces; j++ ) {
			unsigned int nbTriInFan = vertsPerFace - 2;
			for ( unsigned int k = 0; k < nbTriInFan; k++ ) {
