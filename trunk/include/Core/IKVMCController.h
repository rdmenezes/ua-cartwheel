...
private:
	...
	int swingKneeIndex;
	CartWheel::Physics::Joint *swingKnee, *swingHip;
	...
	CartWheel::Math::Vector3d ffRootTorque;

	/**
		determines if there are any heel/toe forces on the given RB
	*/
	void getForceInfoOn(CartWheel::Physics::RigidBody* rb, DynamicArray<CartWheel::Physics::ContactPoint> *cfs, bool* heelForce, bool* toeForce);
...
public:
	...
	CartWheel::Math::Trajectory1d swingFootTrajectorySagittal;
	CartWheel::Math::Trajectory1d swingFootTrajectoryCoronal;
	CartWheel::Math::Trajectory1d swingFootHeightTrajectory;
	CartWheel::Math::Trajectory1d swingFootTrajectoryDeltaSagittal;
	CartWheel::Math::Trajectory1d swingFootTrajectoryDeltaCoronal;
	CartWheel::Math::Trajectory1d swingFootTrajectoryDeltaHeight;
	//this is the vector that specifies the plane of rotation for the swing leg, relative to the root...
	CartWheel::Math::Vector3d swingLegPlaneOfRotation;
...
public:
	...
	*/
	...
	inline void setSwingFootTrajectoryDeltaSagittal( const CartWheel::Math::Trajectory1d& traj ) {
		swingFootTrajectoryDeltaSagittal.copy( traj );
		notifyObservers();
	}
	inline const CartWheel::Math::Trajectory1d& getSwingFootTrajectoryDeltaSagittal() const {
		return swingFootTrajectoryDeltaSagittal;
	}

	inline void setSwingFootTrajectoryDeltaCoronal( const CartWheel::Math::Trajectory1d& traj ) {
		swingFootTrajectoryDeltaCoronal.copy( traj );
		notifyObservers();
	}
	inline const CartWheel::Math::Trajectory1d& getSwingFootTrajectoryDeltaCoronal() const {
		return swingFootTrajectoryDeltaCoronal;
	}

	inline void setSwingFootTrajectoryDeltaHeight( const CartWheel::Math::Trajectory1d& traj ) {
		swingFootTrajectoryDeltaHeight.copy( traj );
		notifyObservers();
	}
	inline const CartWheel::Math::Trajectory1d& getSwingFootTrajectoryDeltaHeight() const {
		return swingFootTrajectoryDeltaHeight;
	}

	inline CartWheel::Math::Vector3d computeSwingFootDelta( double phiToUse = -1, int stanceToUse = -1 ) {
		...
		return CartWheel::Math::Vector3d(
	...
	void setBehaviour( BehaviourController* behaviour_disown ) {
		if( this->behaviour != NULL )
			CartWheel::Util::throwError( "Behaviour already set!" );
	...
	*/
	CartWheel::Math::Vector3d computeIPStepLocation();
	...
	*/
//	void getDesiredSwingAngleLocation(CartWheel::Math::Point3d* target, CartWheel::Math::Point3d* futureTarget, double dt);

	/**
		This method is used to compute the torques
	*/
	virtual void computeTorques(DynamicArray<CartWheel::Physics::ContactPoint> *cfs);
	...
	*/
	CartWheel::Math::Point3d getSwingFootTargetLocation(double t, const CartWheel::Math::Point3d& com, const CartWheel::Math::Quaternion& charFrameToWorld);
	...
	*/
	void computeIKQandW(int parentJIndex, int childJIndex, const CartWheel::Math::Vector3d& parentAxis, const CartWheel::Math::Vector3d& parentNormal,
			const CartWheel::Math::Vector3d& childNormal, const CartWheel::Math::Vector3d& childEndEffector, const CartWheel::Math::Point3d& wP,
			bool computeAngVelocities, const CartWheel::Math::Point3d& futureWP, double dt);
	...
	*/
	void computeLegTorques(int ankleIndex, int kneeIndex, int hipIndex, DynamicArray<CartWheel::Physics::ContactPoint> *cfs);

	void COMJT(DynamicArray<CartWheel::Physics::ContactPoint> *cfs);

	/**
		This method is used to compute the force that the COM of the character should be applying.
	*/
	CartWheel::Math::Vector3d computeVirtualForce();
	...
	*/
	void preprocessAnkleVTorque(int ankleJointIndex, DynamicArray<CartWheel::Physics::ContactPoint> *cfs, CartWheel::Math::Vector3d *ankleVTorque);
