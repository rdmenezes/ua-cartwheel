...
public:
	...
	*/
	int advanceInTime(double dt, DynamicArray<CartWheel::Physics::ContactPoint> *cfs);
	...
	*/
	...
	inline CartWheel::Math::Quaternion getCharacterFrame(){
		if (primaryControllerIndex >= 0 && primaryControllerIndex < (int)controllers.size())
			return controllers[primaryControllerIndex]->getCharacterFrame();
		return CartWheel::Math::Quaternion();
	}
