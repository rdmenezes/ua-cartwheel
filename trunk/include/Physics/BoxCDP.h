...
private:
	...
	CartWheel::Math::Point3d p1, p2;

public:
	BoxCDP(const CartWheel::Math::Point3d& point1, const CartWheel::Math::Point3d& point2, RigidBody* theBody = NULL) :
	...
	*/
	...
	const CartWheel::Math::Point3d& getPoint1() const { return p1; }
	inline void setPoint1( const CartWheel::Math::Point3d& point1 ) { p1 = point1; }
	const CartWheel::Math::Point3d& getPoint2() const { return p2; }
	inline void setPoint2( const CartWheel::Math::Point3d& point2 ) { p2 = point2; }

	/**
		return the center of the box, expressed in local coordinates
	*/
	inline CartWheel::Math::Point3d getCenter(){
		return CartWheel::Math::Point3d((p1.x+p2.x)/2, (p1.y+p2.y)/2, (p1.z+p2.z)/2);
	}
...
/*
		...
		for (unsigned int i=oldContactCount;i<cps->size();i++)
			((*cps)[i]).n *= -1;
		return nContacts;
*/
