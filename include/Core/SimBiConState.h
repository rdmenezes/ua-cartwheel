...
namespace CartWheel {
    namespace Core {
         ...
         */
        class TrajectoryComponent : public CartWheel::Util::Observable {
        public:
            //this is the array of basis functions that specify the trajectories for the sagittal plane.
            CartWheel::Math::Trajectory1d baseTraj;
            ...
            CartWheel::Math::Trajectory1d dTrajScale;

            //this trajectory can be used to scale the base trajectory up or down as a function of the distance from the
            //expected d-value. Used, for instance, to limit the contribution of the toe-off if not at steady state.
            CartWheel::Math::Trajectory1d vTrajScale;
            ...
            CartWheel::Math::Vector3d rotationAxis;
             ...
             */
            TrajectoryComponent() {
                rotationAxis = CartWheel::Math::Vector3d();
            ...
            inline void setRotationAxis(const CartWheel::Math::Vector3d& axis) {
                rotationAxis = axis;
                notifyObservers();
            }

            inline const CartWheel::Math::Vector3d& getRotationAxis() {
                return rotationAxis;
            }
            ...
            inline void setReverseOnStance(int stance) {
                if (stance == ROS_LEFT) {
                    ...
                    CartWheel::Util::throwError("Invalid state stance!");
                notifyObservers();
            }
            ...
            inline void setBaseTrajectory(const CartWheel::Math::Trajectory1d& traj) {
                baseTraj.copy(traj);
                notifyObservers();
            }

            inline const CartWheel::Math::Trajectory1d& getBaseTrajectory() const {
                return baseTraj;
            }

            inline void setVTrajScale(const CartWheel::Math::Trajectory1d& traj) {
                vTrajScale.copy(traj);
                notifyObservers();
            }

            inline const CartWheel::Math::Trajectory1d& getVTrajScale() const {
                return vTrajScale;
            }

            inline void setDTrajScale(const CartWheel::Math::Trajectory1d& traj) {
                dTrajScale.copy(traj);
                notifyObservers();
            }

            inline const CartWheel::Math::Trajectory1d& getDTrajScale() const {
                return dTrajScale;
            }
             ...
             */
            inline CartWheel::Math::Quaternion evaluateTrajectoryComponent(SimBiController* con, CartWheel::Physics::Joint* j, int stance, double phi, const CartWheel::Math::Vector3d& d, const CartWheel::Math::Vector3d& v, bool bareTrajectory = false) {
                ...
                return CartWheel::Math::Quaternion::getRotationQuaternion(baseAngle + feedbackValue, rotationAxis);
            }

            /**
                    this method is used to evaluate the feedback contribution, given the current phase, d and v.
             */
            inline double computeFeedback(SimBiController* con, CartWheel::Physics::Joint* j, double phi, const CartWheel::Math::Vector3d& d, const CartWheel::Math::Vector3d& v) {
                if (bFeedback == NULL)
                    return 0;
             ...
             */
            void updateComponent(SimBiController* con, CartWheel::Physics::Joint* j, CartWheel::Math::Trajectory1d& newDTrajX, CartWheel::Math::Trajectory1d& newDTrajZ, CartWheel::Math::Trajectory1d& newVTrajX, CartWheel::Math::Trajectory1d& newVTrajZ,
                    CartWheel::Math::Trajectory1d* oldDTrajX, CartWheel::Math::Trajectory1d* oldDTrajZ, CartWheel::Math::Trajectory1d* oldVTrajX, CartWheel::Math::Trajectory1d* oldVTrajZ, int nbSamples);
            ...
            void writeScaleTraj(FILE* f, const CartWheel::Math::Trajectory1d& scaleTraj, char type);
         ...
         */
        class ExternalForce : public CartWheel::Util::Observable {
        public:
            ...
            CartWheel::Physics::ArticulatedRigidBody* leftStanceARB;
            //and this is the pointer of the articulated rigid body that the trajectory is associated to if the biped is in a
            //right-side stance
            CartWheel::Physics::ArticulatedRigidBody* rightStanceARB;
            //we'll keep the body name, for debugging purposes
            char bName[100];

            //these are the trajectory to define the force
            CartWheel::Math::Trajectory1d forceX;
            CartWheel::Math::Trajectory1d forceY;
            CartWheel::Math::Trajectory1d forceZ;

            //these are the trajectory to define the force
            CartWheel::Math::Trajectory1d torqueX;
            CartWheel::Math::Trajectory1d torqueY;
            CartWheel::Math::Trajectory1d torqueZ;
            ...
            inline void setForceX(const CartWheel::Math::Trajectory1d& traj) {
                forceX.copy(traj);
                notifyObservers();
            }

            inline void setForceY(const CartWheel::Math::Trajectory1d& traj) {
                forceY.copy(traj);
                notifyObservers();
            }

            inline void setForceZ(const CartWheel::Math::Trajectory1d& traj) {
                forceZ.copy(traj);
                notifyObservers();
            }

            inline void setTorqueX(const CartWheel::Math::Trajectory1d& traj) {
                torqueX.copy(traj);
                notifyObservers();
            }

            inline void setTorqueY(const CartWheel::Math::Trajectory1d& traj) {
                torqueY.copy(traj);
                notifyObservers();
            }

            inline void setTorqueZ(const CartWheel::Math::Trajectory1d& traj) {
                torqueZ.copy(traj);
                notifyObservers();
            }

            inline const CartWheel::Math::Trajectory1d& getForceX() const {
                return forceX;
            }

            inline const CartWheel::Math::Trajectory1d& getForceY() const {
                return forceY;
            }

            inline const CartWheel::Math::Trajectory1d& getForceZ() const {
                return forceZ;
            }

            inline const CartWheel::Math::Trajectory1d& getTorqueX() const {
                return torqueX;
            }

            inline const CartWheel::Math::Trajectory1d& getTorqueY() const {
                return torqueY;
            }

            inline const CartWheel::Math::Trajectory1d& getTorqueZ() const {
                return torqueZ;
            }
             ...
             */
            inline CartWheel::Physics::ArticulatedRigidBody* getARB(int stance) {
         ...
         */
        class Trajectory : public CartWheel::Util::Observable {
        public:
            ...
            CartWheel::Math::Trajectory1d strengthTraj;
            ...
            inline void setStrengthTrajectory(const CartWheel::Math::Trajectory1d& traj) {
                strengthTraj.copy(traj);
                notifyObservers();
            }

            inline const CartWheel::Math::Trajectory1d& getStrengthTrajectory() const {
                return strengthTraj;
            }
            ...
            TrajectoryComponent* getTrajectoryComponent(unsigned int index) {
                return components[index];
            }

            unsigned int getTrajectoryComponentCount() const {
            ...
            /**
                    ...
                    and the d and v values used for feedback.
             */
            inline CartWheel::Math::Quaternion evaluateTrajectory(SimBiController* con, CartWheel::Physics::Joint* j, int stance, double phi, const CartWheel::Math::Vector3d& d, const CartWheel::Math::Vector3d& v, bool bareTrajectory = false) {
                //
                //		if (j && strcmp(j->getName(), "lAnkle") == 0)
                //			tprintf("%2.4lf, %2.4lf\n", d.x, d.z);

                CartWheel::Math::Quaternion q = CartWheel::Math::Quaternion(1, 0, 0, 0);
             ...
             */
            void updateComponents(SimBiController* con, CartWheel::Physics::Joint* j, CartWheel::Math::Trajectory1d& newDTrajX, CartWheel::Math::Trajectory1d& newDTrajZ, CartWheel::Math::Trajectory1d& newVTrajX, CartWheel::Math::Trajectory1d& newVTrajZ,
                    CartWheel::Math::Trajectory1d* oldDTrajX, CartWheel::Math::Trajectory1d* oldDTrajZ, CartWheel::Math::Trajectory1d* oldVTrajX, CartWheel::Math::Trajectory1d* oldVTrajZ, int nbSamples) {
         ...
         */
        class SimBiConState : public CartWheel::Util::Observable {
        ...
        private:
            ...
            CartWheel::Math::Trajectory1d* dTrajX;
            CartWheel::Math::Trajectory1d* dTrajZ;

            //this is the trajectory for the zero value of the feedback v
            CartWheel::Math::Trajectory1d* vTrajX;
            CartWheel::Math::Trajectory1d* vTrajZ;
        ...
        public:
             ...
             */
            ~SimBiConState(void) {
                for (unsigned int i = 0; i < sExternalForces.size(); i++)
                    delete sExternalForces[i];
                for (unsigned int i = 0; i < sTraj.size(); i++)
                    delete sTraj[i];
             ...
             */
            inline ExternalForce* getExternalForce(unsigned int idx) {
            ...
            /**
                    Access a given trajectory
             */
            inline Trajectory* getTrajectory(unsigned int idx) {
            ...
            inline void setDTrajX(CartWheel::Math::Trajectory1d* traj1d_disown) {
                if (dTrajX != NULL) delete dTrajX;
                dTrajX = traj1d_disown;
                notifyObservers();
            }

            inline void setDTrajZ(CartWheel::Math::Trajectory1d* traj1d_disown) {
                if (dTrajZ != NULL) delete dTrajZ;
                dTrajZ = traj1d_disown;
                notifyObservers();
            }

            inline void setVTrajX(CartWheel::Math::Trajectory1d* traj1d_disown) {
                if (vTrajX != NULL) delete vTrajX;
                vTrajX = traj1d_disown;
                notifyObservers();
            }

            inline void setVTrajZ(CartWheel::Math::Trajectory1d* traj1d_disown) {
                if (vTrajZ != NULL) delete vTrajZ;
                vTrajZ = traj1d_disown;
                notifyObservers();
            }

            inline const CartWheel::Math::Trajectory1d* getDTrajX() const {
                return dTrajX;
            }

            inline const CartWheel::Math::Trajectory1d* getDTrajZ() const {
                return dTrajZ;
            }

            inline const CartWheel::Math::Trajectory1d* getVTrajX() const {
                return vTrajX;
            }

            inline const CartWheel::Math::Trajectory1d* getVTrajZ() const {
                return vTrajZ;
            }
             ...
             */
            void updateDVTrajectories(SimBiController* con, CartWheel::Physics::Joint* j, CartWheel::Math::Trajectory1d& newDTrajX,
                    CartWheel::Math::Trajectory1d& newDTrajZ, CartWheel::Math::Trajectory1d& newVTrajX, CartWheel::Math::Trajectory1d& newVTrajZ, int nbSamples = 100);
             ...
             */
            static void readTrajectory1d(FILE* f, CartWheel::Math::Trajectory1d& result, int endingLineType);

            static void writeTrajectory1d(FILE* f, CartWheel::Math::Trajectory1d& result, int startingLineType, int endingLineType);
