#pragma once
#ifndef CARTWHEEL_GLMESH_H
#define CARTWHEEL_GLMESH_H
	...
	*/
	int addPoly(GLIndexedPoly &p){
		if (p.indexes.size()!=this->polyVertexCount)
			return -1;
		for (unsigned int i=0;i<p.indexes.size();i++)
...
private:
	...
	*/
	GLPolyIndexList* getIndexList(size_t vertexCount){
		for (unsigned int i=0;i<categories.size();i++){
...
public:
	...
	*/	
	unsigned int renderToObjFile( FILE* fp, unsigned int vertexIdxOffset, CartWheel::Math::TransformationMatrix toWorld );
...
// Instantiate used STL classes
//GLUTILS_TEMPLATE( DynamicArray<CartWheel::GL::VertexInfo> )
//GLUTILS_TEMPLATE( DynamicArray<CartWheel::GL::SharedVertexInfo*> )
//GLUTILS_TEMPLATE( DynamicArray<CartWheel::GL::GLPolyIndexList*> )
//GLUTILS_TEMPLATE( DynamicArray<CartWheel::GL::GLMesh*> )
#endif
