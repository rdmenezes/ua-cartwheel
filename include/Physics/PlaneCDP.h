...
private:
	...
	CartWheel::Math::Plane p;
	//and this is the plane expressed in world coordinates
	CartWheel::Math::Plane wP;
	
public:
	PlaneCDP(const CartWheel::Math::Vector3d& normal, const CartWheel::Math::Point3d& origin, RigidBody* theBody = NULL ) :
	...
	virtual void draw();
	const CartWheel::Math::Vector3d& getNormal() const {return p.n;}
	inline void setNormal( const CartWheel::Math::Vector3d& normal ) { p.n = normal; }
	const CartWheel::Math::Point3d& getOrigin() const {return p.p;};
	inline void setOrigin( const CartWheel::Math::Point3d& origin ) { p.p = origin; }
